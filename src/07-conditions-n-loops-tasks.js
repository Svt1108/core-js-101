...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    if (Array.from(str.matchAll(str[i])).length === 1) return str[i];
  }
  return null;
}
...
function getDigitalRoot(num) {
  const arr = [];
  let numToCalculate = num;
  const len = num.toString().length;
  if (len === 1) return numToCalculate;
  for (let i = 0; i < len; i += 1) {
    arr.push(numToCalculate % 10);
    numToCalculate = Math.floor(numToCalculate / 10);
  }
  const checkSum = arr.reduce((sum, cur) => sum + cur, 0);
  return getDigitalRoot(checkSum);
}
...
function isBracketsBalanced(str) {
  let str2 = str;
  const brackets1 = new RegExp('\\[]', 'g');
  const brackets2 = new RegExp('<>', 'g');
  const brackets3 = new RegExp('{}', 'g');
  const brackets4 = new RegExp('\\(\\)', 'g');
  while (
    str2.includes('[]')
        || str2.includes('()')
        || str2.includes('{}')
...
function toNaryString(num, n) {
  const arr = [];
  let numToCalculate = num;
  while (numToCalculate > 0) {
    arr.push(numToCalculate % n);
    numToCalculate = Math.floor(numToCalculate / n);
  }
  arr.push(numToCalculate);
  return Number(arr.reverse().join(''));
}

/**
 * Returns the commonPart directory path for specified array of full filenames.
 *
 * @param {array} pathes
 * @return {string}
 *
...
function getCommonDirectoryPath(pathes) {
  ...
  if (pathes.filter((item) => item.startsWith('/')).length === pathes.length) commonPart = '/';
  else if (pathes.filter((item) => item.startsWith('/')).length === 0) commonPart = '';
  else return '';
  ...
  for (let i = 0; i <= arr.length; i += 1) {
    if (flag === 1) break;
    commonResult = commonPart;
    commonPart = `${commonPart + arr[i]}/`;
    for (let j = 0; j < pathes.length; j += 1) {
      if (!pathes[j].startsWith(commonPart)) {
        flag = 1;
        break;
      }
    }
